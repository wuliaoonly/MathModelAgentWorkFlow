    ...
    async def run(
        ...
        if (
            ...
            if tool_call.function.name == "search_papers":
                ...
                # 处理 response_content 为 None 的情况
                if response_content is None:
                    logger.warning("工具调用后返回的 content 为 None，使用默认消息")
                    response_content = "已完成文献搜索，但未能生成内容。"
        else:
            response_content = response.choices[0].message.content
            
            # 处理 response_content 为 None 的情况
            if response_content is None:
                logger.warning("WriterAgent 返回的 content 为 None，使用默认消息")
                response_content = "内容生成中遇到问题，请稍后重试。"
